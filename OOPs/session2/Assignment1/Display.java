...
public class Display {
	...
	protected UserHandling generateUI(UserHandling user)
	{
		...
		{
			String outputString=this.questions.questionNumber.get(index)+"  ---  "+this.questions.question.get(index)+"  ---  "+this.questions.questionType.get(index)+"  ---  ";
	...
	//This function takes input from the user and also check for validation of response
	...
	protected void generateResult(UserHandling[] userHandlers,int[] indicesOfSingleSelect,Questions questions)
	{
		...
		else
		{
			...
			{
				output=questions.question.get(index).toString()+", Single Select,"+questions.optionsAvailable.get(index);	
